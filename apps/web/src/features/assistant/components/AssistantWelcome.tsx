...
export function AssistantWelcome({ onPick }: AssistantWelcomeProps) {
  return (
    <div className='animate-fade-in flex flex-col items-center px-4 py-10 text-center'>
      <AssistantAvatar size={88} />

      <h2 className='mt-5 text-xl font-semibold text-fg'>Hi, I&rsquo;m Tia</h2>
      ...
      <div className='stagger mt-6 flex max-w-lg flex-wrap justify-center gap-2'>
        {SUGGESTIONS.map((s) => (
          <button
            key={s.label}
            type='button'
            onClick={() => onPick(s.prompt)}
            className='group inline-flex items-center gap-1.5 rounded-full border border-border bg-surface px-3.5 py-2 text-sm font-medium text-fg-body shadow-xs transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:bg-primary-subtle hover:text-primary hover:shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-hover'
          >
            <span
              aria-hidden
              className='h-1.5 w-1.5 rounded-full bg-primary/50 transition-colors group-hover:bg-primary'
            />
            {s.label}
          </button>
        ))}
